...
import { detailTransition } from "./details.animation";
...
export class DetailComponent implements OnInit {
  ...
  handleDueDateChange(date: Date): void {
    ...
    if (dueAt && lessThanADay(dueAt)) {
      this.message.warning('this project will be ending in 24 hours', {
        nzDuration: 6000
      });
    }
    this.checkDate();
  ...
  private checkDate(): void {
    ...
    if (dueAt && planAt && floorToDate(planAt) > dueAt) {
      this.message.warning('Are you sure you want to do this project after deadline?', {
        nzDuration: 6000
      });
    }
  }
  ...
  clickSwitch(): void {
    ...
    if (!this.currentTodo.planAt) {
      this.message.warning('did not set plan date');
      return;
    }
